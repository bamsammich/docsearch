...
#
#   SECOND_USER_DOC=<doc ID> spike/postgres/run.sh
#
# Run from the repository root, with the docsearch stack up so its database
# can be copied out. SECOND_USER_DOC names the document the second user loads
# three copies of. Starts two throwaway Postgres containers bound to
...
second=${SECOND_USER_DOC:?set SECOND_USER_DOC to the doc ID the second user loads}
...
isolate() { # engine url partition-flag label
  ...
  run -engine "$1" -pg "$2" $3 -user bob -only "$second" -times 3 load > /dev/null
