...
#: The schema, as DDL, read from the one file that holds it.
#:
#: Three things consume it and none of them are generated from another: this
#: module creates databases with it, sqlc types the Go store's queries from it,
#: and the Go store tests build their index from it. It is a .sql file rather
#: than a string literal here so that all three read the same bytes.
