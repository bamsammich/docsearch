...
| homelab default PostgreSQL 17 | supported | image ships 18 |
...
the body counts twice at full strength. On a Rubia query, "tDCS" sits in the
heading of every chunk in its section and in some bodies; it outscored the
rare body word "soltaninejad" that identifies the right chunk, and the target
fell from rank 1 to rank 4.
...
A second user ("bob", three copies of the grandMA2 manual) loaded into one
...
2. Partition chunks by user, `PARTITION BY LIST (user_id)`, one partition per
   user. The homelab has a handful of users; thousands of partitions is a
   different design.
...
- **The operator.** The image was built and run under plain Docker. Loading
  ...
  on the homelab is untested, and the homelab's PostgreSQL 17 default is
  inferred from the chart version, because `kubectl` could not reach the
  cluster.
