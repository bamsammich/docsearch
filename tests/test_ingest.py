...
from docsearch.ingest import ingest_file, slugify
...
def test_reingest_unchanged_file_is_a_noop(conn: sqlite3.Connection, md_file: Path) -> None:
    first = ingest_file(conn, md_file)
    second = ingest_file(conn, md_file)
...
def test_retitled_file_replaces_rather_than_forking_a_second_doc(
    ...
    first = ingest_file(conn, md_file, title="Original Title")
    second = ingest_file(conn, md_file, title="Totally Different Title")
    assert second.doc_id == first.doc_id
    assert conn.execute("SELECT COUNT(*) c FROM documents").fetchone()["c"] == 1
...
def test_distinct_documents_get_distinct_ids(
    ...
    a = ingest_file(conn, md_file, title="Same Name")
    b = ingest_file(conn, txt_file, title="Same Name")
...
def test_html_and_text_adapters_ingest(
    ...
    html = ingest_file(conn, html_file)
    text = ingest_file(conn, txt_file)
